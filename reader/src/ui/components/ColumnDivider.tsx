...
import type { DrawnColumn } from "../hooks/column-widths"
...
export function ColumnDivider({ name, edge, column }: ColumnDividerProps) {
  ...
  function move(event: PointerEvent<HTMLDivElement>) {
    ...
    column.resize(drag.current.width + widening(event.clientX - drag.current.from))
  }
  ...
  function press(event: KeyboardEvent<HTMLDivElement>) {
    if (event.key === "ArrowLeft" || event.key === "ArrowRight") {
      ...
      column.resize(column.width + widening(event.key === "ArrowRight" ? STEP : -STEP))
  ...
  return (
    ...
    <div
      ...
      style={edge === "right" ? { left: `${column.width - 3}px` } : { right: `${column.width - 3}px` }}
      role="separator"
